...
from apparatus.evidence import action_schema, source_record
from apparatus.regions import MAX_READ_BYTES, Region, build_regions
...
class ActionEnvironment:
    ...
    def __init__(self, cell: str, candidate_root: Path, submission_root: Path) -> None:
        ...
        source_candidate = parent_candidate_path(cell)
        shutil.copy2(source_candidate, self.candidate_root / TARGET)
        self.initial_snapshot = snapshot_directory(self.candidate_root)
        expected_snapshot = snapshot_directory(source_candidate.parent)
        if self.initial_snapshot["candidate_id"] != expected_snapshot["candidate_id"]:
            raise RuntimeError("materialized candidate identity does not match the donor endpoint")
        self.target_bytes = (self.candidate_root / TARGET).read_bytes()
        self.exact_reopens: dict[str, dict[str, Any]] = {}
    ...
    def _build_source_records(self) -> dict[str, dict[str, Any]]:
        return {path: source_record(path) for path in SOURCE_PATHS}
    ...
    def _read_region(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        regions = {region.region_id: region for region in build_regions(current)}
        region = regions.get(action["region_id"])
        if region is None:
            raise ActionRejected("unknown_region", "region_id is not in the current mechanical outline")
        raw = region.raw
        return {
            "accepted": True,
            "candidate_id": self.snapshot()["candidate_id"],
    ...
    def _continue_lines(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        if binding["source_kind"] == "candidate":
            current = self.snapshot()
            path, _ = self._resolve(binding["path"])
            if (
                current["candidate_id"] != binding["candidate_id"]
                or not path.is_file()
                or sha256_bytes(path.read_bytes()) != binding["file_sha256"]
            ):
                raise ActionRejected("invalid_or_stale_cursor", "candidate cursor basis is no longer current")
        ...
        raise ActionRejected("unsupported_cursor_kind", "cursor source kind is unsupported")
