...
from apparatus.constants import CONTEXT_TOKENS, LLAMA_BUILD, MODEL_ALIAS, ROOT, STAGE_A_MAXIMUM_CALLS
...
from apparatus.stage_a import run_stage_a
...
def server_arguments(server: Path, model: Path, host: str, port: int, runtime: Path) -> list[str]:
    ...
    return [
        str(server), "-m", str(model), "--alias", MODEL_ALIAS, "--host", host, "--port", str(port),
        "--gpu-layers", "all", "--fit", "off", "-c", "25000", "--flash-attn", "on",
        "-ctk", "q8_0", "-ctv", "q8_0", "--kv-unified", "-b", "512", "-ub", "256",
        "--threads", "7", "--threads-batch", "8", "--parallel", "1", "--cache-prompt",
        "--cache-ram", "0", "--slot-save-path", str(slots), "--no-context-shift", "--jinja",
        "--reasoning", "off", "--reasoning-format", "deepseek", "--reasoning-budget", "0",
        "--no-reasoning-preserve", "--temp", "0.7", "--top-p", "0.8", "--top-k", "20",
        "--min-p", "0.0", "--presence-penalty", "1.5", "--repeat-penalty", "1.0",
        "--metrics", "--slots", "--no-webui", "--no-mmproj", "--verbose", "--log-file", str(runtime / "llama-server.log"),
    ]
...
def verify_endpoint(base_url: str, model: Path) -> dict[str, Any]:
    ...
    if errors:
        raise RuntimeError("endpoint identity failed: " + "; ".join(errors))
    return props


def execute(run_id: str, server: Path, model: Path, host: str, port: int) -> dict[str, Any]:
    require_authorization("stage-a", STAGE_A_MAXIMUM_CALLS)
    head = require_clean_head()
    runtime_files = verify_runtime_files(server.resolve(), model.resolve())
    existing = llama_processes()
    if existing:
        raise RuntimeError(f"llama-server isolation failed: {existing}")
    if port_open(host, port):
        raise RuntimeError(f"intended port is open: {host}:{port}")
    runtime = ROOT / ".cache" / "runtime" / run_id
    if runtime.exists():
        raise RuntimeError(f"runtime path exists: {runtime}")
    runtime.mkdir(parents=True)
    arguments = server_arguments(server.resolve(), model.resolve(), host, port, runtime)
    lifecycle: dict[str, Any] = {"schema_version": "bounded-progress-state-stage-a-runtime-v0", "run_id": run_id, "standalone_commit": head, "started_at_unix": time.time(), "gpu_before": gpu_state(), "existing_llama_processes_before": existing, "port_open_before": False, "server_arguments": arguments, "passed": False}
    ...
    try:
        with (runtime / "stdout.log").open("wb") as stdout, (runtime / "stderr.log").open("wb") as stderr:
            ...
            result = run_stage_a(run_id, base_url, custody)
            run_created = True
            lifecycle["run_result"] = result
            lifecycle["passed"] = result["passed_apparatus_integrity"] and result["model_calls"] <= STAGE_A_MAXIMUM_CALLS
    finally:
