...
from apparatus.stage_b_preflight import run_stage_b_preflight
...
def execute(server: Path, tokenizer: Path, host: str, port: int) -> dict:
    ...
    if port_open(host, port):
        raise RuntimeError(f"Stage B preflight port already open: {host}:{port}")
    cache = ROOT / ".cache" / f"stage-b-tokenizer-{time.time_ns()}"
    ...
    lifecycle = {"schema_version": "bounded-progress-state-stage-b-preflight-runtime-v0", "measured_inference": False, "chat_completions_called": False, "server_arguments": arguments, "port": port, "gpu_used": False, "released": False}
    try:
        with (cache / "stdout.log").open("wb") as stdout, (cache / "stderr.log").open("wb") as stderr:
            ...
            receipt = run_stage_b_preflight(f"http://{host}:{port}")
            lifecycle["preflight_passed"] = bool(receipt["verification_passed"])
    finally:
        ...
        write_json(ROOT / "provenance" / "STAGE_B_PREFLIGHT_RUNTIME_RELEASE.json", lifecycle)
    return lifecycle
